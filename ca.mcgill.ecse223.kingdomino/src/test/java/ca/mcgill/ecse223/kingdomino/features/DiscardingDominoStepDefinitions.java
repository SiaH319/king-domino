...
public class DiscardingDominoStepDefinitions {
	...
	public void the_current_player_is_not_the_last_player_in_the_turn() {
		...
		if(game.getNextDraft()!=null) {
			game.getPlayer(2).setDominoSelection(new DominoSelection(game.getPlayer(2), game.getNextDraft().getIdSortedDomino(0), game.getNextDraft()));
			game.getPlayer(3).setDominoSelection(new DominoSelection(game.getPlayer(3), game.getNextDraft().getIdSortedDomino(1), game.getNextDraft()));

		}
		game.getPlayer(1).setDominoSelection(new DominoSelection(game.getPlayer(1), game.getCurrentDraft().getIdSortedDomino(3), game.getCurrentDraft()));//Last Player
		game.getNextPlayer().setDominoSelection(new DominoSelection(game.getNextPlayer(), game.getCurrentDraft().getIdSortedDomino(2), game.getCurrentDraft()));//CurrentPlayer
	...
	public void the_current_player_is_the_last_player_in_the_turn() {
		Game game = KingdominoApplication.getKingdomino().getCurrentGame();
		// current player is last in turn so he has selected the domino with the largest domino id
		// all three other players have selected a domino from the next draft if there is one
		
		if(game.getNextDraft()!=null) {
			game.getPlayer(1).setDominoSelection(new DominoSelection(game.getPlayer(1), game.getNextDraft().getIdSortedDomino(2), game.getNextDraft()));//Last Player
			game.getPlayer(2).setDominoSelection(new DominoSelection(game.getPlayer(2), game.getNextDraft().getIdSortedDomino(0), game.getNextDraft()));
			game.getPlayer(3).setDominoSelection(new DominoSelection(game.getPlayer(3), game.getNextDraft().getIdSortedDomino(1), game.getNextDraft()));

		}
		game.getNextPlayer().setDominoSelection(new DominoSelection(game.getNextPlayer(), game.getCurrentDraft().getIdSortedDomino(3), game.getCurrentDraft()));//CurrentPlayer
